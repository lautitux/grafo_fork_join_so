...
compileStatements parents state =
    case state.current of
        ...
        stmt :: _ ->
            case stmt.value of
                Label _ -> compileStatements parents (advance state)
                Counter ident value ->
                    case Dict.get ident state.counters of
                        Nothing -> compileStatements parents (advance { state | counters = Dict.insert ident value state.counters })
                        Just _ -> Err <| located stmt ("Invalid re-assignment of already assigned counter '" ++ ident ++ "'.")
                Fork lbl ->
                    case goto state lbl of
                        Ok goto_state ->
                            let
                                goto_result = compileStatements parents goto_state
